---
title: "scRNA-seq data processing (blastocyst stage)"
...
counts from the following paper and applies the quality control and 
normalisation pipeline outlined [here](https://f1000research.com/articles/5-2122/v2).
...
We start by loading the raw counts stored in a `SingleCellExperiment` object:

```{r}
load("data/SCE_blk_raw.RData")
```
...
is.mito <- rownames(bl_sce) %in% mc$gene_name
```

```{r echo=FALSE, message=FALSE, include=FALSE}
# Clean up before going on
rm(blakeley, mc)
```
...
```{r}
bl_sce <- calculateQCMetrics(bl_sce, feature_controls = list(Mt = is.mito))
qc <- tibble(tot_counts = bl_sce$total_counts / 1e6,
             expr_genes = bl_sce$total_features_by_counts,
             pct_mt = bl_sce$pct_counts_Mt)
```
...
```{r}
libsize.drop <- isOutlier(bl_sce$total_counts, nmads = 3, 
                          type = "lower", log = TRUE)
feature.drop <- isOutlier(bl_sce$total_features_by_counts, nmads = 3, 
                          type = "lower", log = TRUE)
mito.drop <- isOutlier(bl_sce$pct_counts_Mt, nmads = 3, type = "higher")

bl_sce <- bl_sce[, !(libsize.drop | feature.drop | mito.drop)]
```
...
anno <- select(org.Hs.eg.db, keys = rownames(bl_sce), keytype = "SYMBOL", 
               column = "ENSEMBL")
ensembl <- anno$ENSEMBL[match(rownames(bl_sce), anno$SYMBOL)]
assignments <- cyclone(bl_sce, hs.cycle, gene.names = ensembl)
bl_sce$cycle <- factor(
  case_when(
...
colData(bl_sce) <- cbind(colData(bl_sce), assignments$scores)
```

The result of phase assignment for each cell is shown below. Normally, 
...
```{r}
ave.counts <- rowMeans(counts(bl_sce))
keep <- ave.counts >= 1
sum(keep)
```
...
represents the bulk of moderately expressed genes while the rectangular 
component around 0 corresponds to lowly expressed genes. The filter threshold 
should cut the distribution at some point along the rectangular component to 
remove the majority of low-abundance genes, which it does in this case.
...
with expected biology, while the absence of ribosomal proteins and/or the 
...
```{r}
bl_sce <- computeSumFactors(bl_sce, sizes = seq(3, 18, 3))
summary(sizeFactors(bl_sce))
```
...
library(scales)
tibble(sf = sizeFactors(bl_sce), ls = bl_sce$total_counts/1e6) %>% 
...
```{r}
bl_sce <- normalize(bl_sce)
```

Finally, we impute dropout events with [DrImpute](https://doi.org/10.1186/s12859-018-2226-y). 
...
```{r message=FALSE}
assays(bl_sce)$logcounts_old <- assays(bl_sce)$logcounts
assays(bl_sce)$logcounts <- DrImpute::DrImpute(assays(bl_sce)$logcounts, 
                                               ks = 6:10)
```
...
```{r}
des_mat <- model.matrix(~ G1 + G2M, colData(bl_sce))  
var.fit <- trendVar(bl_sce, method = "loess", use.spikes = FALSE, 
                    design = des_mat)
var.out <- decomposeVar(bl_sce, var.fit)
```
...
```{r, message=FALSE}
set.seed(100)
hvg.cor <- correlatePairs(bl_sce, subset.row = rownames(hvg.out))
...
hvg.mat <- assays(bl_sce)$logcounts[top.hvg, ]
pc <- prcomp(t(hvg.mat))

res <- tibble(x = pc$x[, 1], y = pc$x[, 2], cond = bl_sce$sample_alias)
  
pc1_var <- round(summary(pc)$importance[2, 1] * 100, 2)
pc2_var <- round(summary(pc)$importance[2, 2] * 100, 2)

col_pal <- c("#e41a1c", "#377eb8", "#4daf4a")

ggplot(res, aes(x, y, colour = cond)) + geom_point(size = 2.5) + 
...
```{r hmap, echo = FALSE, fig.cap = "**Figure 8.** Heatmap of the data based on correlated HVGs."}
library(pheatmap)

ann <- data.frame(cell_type = factor(bl_sce$sample_alias),
                  row.names = bl_sce$sample_accession)
heat.vals  <- hvg.mat - rowMeans(hvg.mat)  # center expression around the mean
colnames(heat.vals) <- bl_sce$sample_accession
h <- pheatmap(heat.vals, annotation_col = ann, cutree_rows = 3, cutree_cols = 3,
...
bl_hvg_corr <- top.hvg
bl_hvg_all <- hvg.out
save(bl_sce, bl_hvg_corr, bl_hvg_all, file = "data/SCE_blk.RData")
```
